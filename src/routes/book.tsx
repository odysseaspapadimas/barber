...
function RouteComponent() {
  ...
  async function handleSubmit(data: any) {
    if (!selectedService || !chosenSlot) return alert("Select a service and slot first");
    try {
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-2">
            ...
            <section className="bg-card border-2 border-border rounded-xl p-6">
              ...
              {!selectedService ? (
                <div className="text-muted-foreground">Please select a service to see slots.</div>
              ) : !available ? (
                <div className="text-muted-foreground">Loading slots…</div>
              ) : available.slots.length === 0 ? (
                <div className="text-muted-foreground">No available slots for that day.</div>
              ) : (
                <div className="flex gap-2 flex-wrap">
                  {available.slots.map((slot: any) => (
                    <button
                      ...
                      className={`px-4 py-2 rounded-lg ${chosenSlot === slot.startTs ? 'bg-primary text-primary-foreground' : 'bg-muted text-foreground'}`}
                    >
